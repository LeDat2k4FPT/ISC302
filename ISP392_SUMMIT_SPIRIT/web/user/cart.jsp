...
//            }
        ...
        <div class="main-wrapper">
            <div class="cart-section">
                <h2>Shopping cart</h2>
                <p>You have <%= cart.getCartItems().size()%> item<%= cart.getCartItems().size() > 1 ? "s" : ""%> in your cart</p>                <% for (CartItemDTO item : cart.getCartItems()) {
...
<!--                            <input type="number" name="quantity_<%= uniqueKey%>" id="qty_<%= uniqueKey%>" value="<%= quantity%>" min="1" max="<%= stock%>" readonly>-->
                            <input type="number"
                                   ...
                                   min="1"
                                   max="<%= stock%>"
                                   oninput="handleManualInput('<%= uniqueKey%>', <%= stock%>)">
